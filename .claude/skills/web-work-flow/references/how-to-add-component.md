...
**约定模板**(本仓库已有先例:`react-components/src/shortcut-library/`、`react-components/src/car-battle/`、`react-components/src/gaussian-splat-viewer/` 都用此结构):
...
**示例对照**(取自 `react-components/src/shortcut-library/`):
...
**Vue 侧的真实例子**(`packages/vue-components/src/china-map/`、`heavy-chart/`、`button/` 之外的大型组件)也走同样的 `src/` 子目录约定。
...
3. **绝对不要在 `<id>/` 下另开 `index.ts` 作为 barrel**——`import.meta.glob` 只扫 `<id>/index.{vue,tsx}` 一层,barrel 多此一举;`import.meta.glob('../../../../packages/{vue,react}-components/src/*/index.{vue,tsx}')` 字面量写死。
4. **`component.config.ts` 永远在组件根目录,不在 `src/` 里**——`manifestPlugin` 扫 `packages/*/src/*/component.config.ts`(`packages/manifest-generator/src/vite-plugin.ts` 的 `componentRoots`),把 config 挪进 `src/` 会导致卡片不显示。
5. **`__tests__/` 可放组件根目录或 `src/` 同级**——见 `shortcut-library/__tests__/`。
