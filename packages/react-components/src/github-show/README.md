...
- **产出为可选文本列**:可写解释文字,含 http 自动渲染为可点击链接,可修改、可清空(v1.3.0 起由「线上地址」更名,旧数据自动迁移)
- **行排序**:编辑模式操作列提供上移 / 下移,调整项目展示顺序,随保存自动同步
- **列类型**:自定义列支持「文本」(含 http 自动渲染链接)、「多选」(chip 标签,回车/逗号添加)与「数字」(数值输入,展示页右对齐)
- **按数字排序**:展示页列头点击任意列排序;数字列按数值大小排序,用于快速控制展示顺序(如填"优先级"列)
- **展示页显示控制**:列设置弹窗可切换自定义列在展示页显示 / 隐藏(编辑页始终显示全部列),PDF 导出与展示页一致
- **列扩展 CRUD**:列设置弹窗可新增自定义列(选类型)、重命名、删除(列数据一并删除)
- **链接可点击跳转**:合法 http(s) 链接(GitHub / 产出 / 文本列内 URL)渲染为可点击链接,新标签页打开
...
interface GithubShowColumn {
  id: string;
  title: string;
  type: 'text' | 'multi-select' | 'number'; // 列类型;multi-select 值存 JSON 数组字符串
  createdAt: number;
...
interface GithubShowRow {
  ...
  output: string;          // 可选文本列:产出 / 成果链接或说明,含 http 自动可点
  values: Record<string, string>; // 自定义列值,key = column.id;number 列存数字字符串
  createdAt: number;
  updatedAt: number;
}
```
...
src/
  ...
  components/NumberCell.tsx         # 数字列输入单元格
