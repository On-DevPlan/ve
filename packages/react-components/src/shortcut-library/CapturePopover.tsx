...
interface Props {
  onConfirm: (combo: KeyStroke[]) => void;
  onCancel: () => void;
  initial?: KeyStroke[];
}

export default function CapturePopover({ onConfirm, onCancel, initial = [] }: Props) {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.code === 'Enter') {
        e.preventDefault();
        e.stopPropagation();
        if (combo.some((k) => !k.isModifier)) onConfirm(combo);
        return;
      }
      // 其它键 → 作为主键加入
