...
const updateClock = () => {
  ...
  if (hrRef.value) hrRef.value.style.transform = `rotateZ(${h + m / 12}deg)`
  if (mnRef.value) mnRef.value.style.transform = `rotateZ(${m}deg)`
  if (scRef.value) scRef.value.style.transform = `rotateZ(${s}deg)`
}
...
<template>
  <div class="demo-wrapper">
    <div class="clock">
      <div class="hour">
        <div class="hr" ref="hrRef"></div>
      </div>
      <div class="min">
        <div class="mn" ref="mnRef"></div>
      </div>
      <div class="sec">
        <div class="sc" ref="scRef"></div>
      </div>
    </div>
  </div>
</template>
...
.demo-wrapper {
  ...
  align-items: center;
  background: linear-gradient(rgb(145, 170, 207), rgb(145, 170, 207));
}
